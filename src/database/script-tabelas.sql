...
FROM
    ...
    JOIN Log_resposta ON Resposta.idResposta = Log_resposta.fkResposta
WHERE
    dtHrResposta >= DATE(NOW() - INTERVAL 30 DAY)
GROUP BY
    nome
...
FROM
    ...
    JOIN Log_resposta ON Resposta.idResposta = Log_resposta.fkResposta
WHERE
    dtHrResposta >= NOW() - INTERVAL 10 DAY
    AND idUsuario = 1
GROUP BY
    nome,
    dtHrResposta
...
SELECT nome, max(altura_hobbits) as maior, min(altura_hobbits) as menor, avg(altura_hobbits) as media
FROM
    usuario
    join alturas_usuarios ON usuario.id = fk_usuario
ORDER BY altura_hobbits;